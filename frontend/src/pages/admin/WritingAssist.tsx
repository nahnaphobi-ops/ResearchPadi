...
export default function WritingAssist() {
  ...
  const getConfidenceColor = (c: string) => {
    switch (c) {
      case 'high': return 'bg-gray-100 text-gray-800 border-gray-200';
  ...
  const getSeverityColor = (s: string) => {
    switch (s) {
      ...
      case 'suggestion': return 'bg-gray-50 border-gray-500';
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition ${
              activeTab === tab.id ? 'bg-gray-100 text-gray-700' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
            {tab.icon} {tab.label}
          </button>
          ...
          {activeTab !== 'citations' && activeTab !== 'pdf-chat' && (
            <div className="bg-white rounded-lg shadow border p-4">
              <textarea
                ...
                className="w-full h-64 p-4 border rounded-lg resize-none focus:ring-2 focus:ring-gray-500"
              />
              <div className="mt-3 flex gap-3">
                {activeTab === 'grammar' && (
                  <button onClick={handleGrammarCheck} disabled={loading || !text.trim()}
                    className="px-6 py-2 bg-gray-800 text-white rounded-lg font-medium hover:bg-gray-900 disabled:bg-gray-400">
              ...
              <div className="divide-y max-h-96 overflow-y-auto">
                {claimAnalyses.map((a, idx) => (
                  <div key={idx} className="p-4">
                    ...
                    {a.suggestions.map((s, si) => <p key={si} className="text-xs text-gray-800 mt-1">• {s}</p>)}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-4 bg-white rounded-lg shadow border p-4">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="text-center p-3 bg-gray-50 rounded">
                  <div className="text-2xl font-bold text-gray-800">{plagiarismReport.totalWords}</div>
            ...
            <div className="bg-white rounded-lg shadow border p-4">
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={handleFormatCitation} disabled={loading}
                  className="px-4 py-2 bg-gray-800 text-white rounded-lg text-sm font-medium hover:bg-gray-900 disabled:bg-gray-400">
                  Format Citation
                </button>
              ...
              {formattedCitation && (
                <div className="mt-4 p-3 bg-gray-50 rounded">
                  ...
                  <p className="text-sm text-gray-800">{formattedCitation.inText}</p>
                ...
                {blueprints.map(bp => (
                  <button key={bp.institutionType} onClick={() => setSelectedBlueprintType(bp.institutionType)}
                    className={`px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition ${
                      selectedBlueprintType === bp.institutionType ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                    }`}>
              ...
              {selectedBlueprint && (
                <div className="space-y-4">
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-gray-800">{selectedBlueprint.description}</p>
                    <p className="text-[10px] text-gray-800 mt-1">Based on {selectedBlueprint.sampleSize} documents</p>
                  </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          {grammarSummary && (
            <div className="bg-white rounded-lg shadow border p-4">
              ...
              <div className="text-center mb-4">
                <div className="text-4xl font-bold text-gray-800">{grammarSummary.score}</div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-gray-800">Suggestions</span><span>{grammarSummary.suggestions}</span></div>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-lg shadow border p-4">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              <div className="flex items-center gap-2"><span className="w-2 h-2 bg-gray-500 rounded-full"></span>Grammar & Style</div>
              <div className="flex items-center gap-2"><span className="w-2 h-2 bg-gray-500 rounded-full"></span>Claim Validation</div>
